...
pdf("Figures/Article/Figure3.pdf",width=8.8/2.54,height=8.8/2.54)
ggplot2::last_plot()
dev.off()
...
p2
# Save with width size of letter
ggsave("Figures/Article/SupplyCountries.png", ggplot2::last_plot(),
       units="cm",dpi=600,
       width=12.1,height=12.1)

# Foreign country supply source --------

# free trade countries
USA_ft_countries <- c("Australia","Bahrain","Canada","Chile","Colombia",
                      "Costa Rica","Dominican Republic","El Salvador",
                      "Guatemala","Honduras","Israel","Jordan",
                      "Korea","Mexico","Morocco","Nicaragua",
                      "Oman","Panama","Peru","Singapore","Japan","United States")

# no recycling yet
df_results %>% 
  filter(t<2051) %>% 
  left_join(deposit,by="Deposit_Name") %>% 
  mutate(free_trade=Country %in% USA_ft_countries) %>% 
  group_by(name,free_trade) %>% 
  reframe(mtons=sum(tons_extracted)/1e3) %>% ungroup() %>% 
  # pivot_wider(names_from = free_trade, values_from = mtons) %>% 
  group_by(name) %>% mutate(total=sum(mtons)) %>% ungroup() %>% 
  mutate(share=mtons/total) %>% 
  filter(free_trade==F)
