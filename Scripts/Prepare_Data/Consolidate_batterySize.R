...
url_save <- "Results/Intermediate Results/%s.csv"

# LOAD DATA ---------
source("Scripts/00-Libraries.R", encoding = "UTF-8")

# dictionary or equivalency for ICCT
eq_country_region <- read_excel("Data/Eq_Countries_ICCT_EVV.xlsx",
                                sheet = "Eq_Country", range = "A1:B188")
names(eq_country_region) <- names(eq_country_region) %>% str_remove("ICCT_")

# for LDV by country 
bat_ldv <- read.csv("Results/Battery/battery_size_country.csv")
# By region
...
eqs <- read_excel("Data/Eq_Countries_ICCT_EVV.xlsx",sheet = "Eq_Country2", range = "A1:B117")
...
chem_SS <- read.csv("Results/Battery/Chemistry_Scenarios/SolidState_scen_region.csv")
chem_SS$chem_scenario <- "Solid State adoption"
chem_SIB <- read.csv("Results/Battery/Chemistry_Scenarios/Sodium_scen_region.csv")
